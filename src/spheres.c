...
void createSphereMesh(Sphere* sphere, const unsigned int sector_count, const unsigned int stack_count)
{
	 ...
	 * vertices_count is taken with reserve of 50 additionaal vertices. */

	sphere->mesh.vertices_count = sizeof(float) * ((sector_count * stack_count + sector_count + 50) * 3);
    sphere->mesh.indices_count  = sizeof(unsigned int) * (sector_count * 6 + (stack_count - 2) * sector_count * 6);

    sphere->mesh.vertices = malloc(sphere->mesh.vertices_count);
    sphere->mesh.normals  = malloc(sphere->mesh.vertices_count);
    sphere->mesh.indices  = malloc(sphere->mesh.indices_count);
